<template>
<ion-page>
    <header-component pageTitle="Edit produit" :pageDefaultBackLink="`/admin/produit/`+produit.id"></header-component>
...
export default ({
    name:"AdminEditProduit",
    created(){
        this.produit =  this.$store.getters.getProduit(this.produit.categorie_id)
    },
    data(){
        return{
            produit: {
    ...
    methods:{
        submitForm(){
            ...
            this.$store.dispatch("editProduit",this.produit)
            setTimeout(() => {  
